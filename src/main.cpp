...
uint256 hashGenesisBlock("0xe8ebce0f803c420b1cae679abe585c0c5287ab72234583b25081ec43f11a88b7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WPCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WPCoin Signed Message:\n";
...
{
    // WPCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 5 * 60; // WPCoin: 5 minutes
static const int64 nTargetSpacing = 60; // WPCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WPCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // WPCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe8ebce0f803c420b1cae679abe585c0c5287ab72234583b25081ec43f11a88b7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "WPCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04be790543a1b70e0773c1a357ae11f9465c448a320c5f4f8932a2afbb44d9a2f6fc00f5f582f2a7a7edd96f97d344b86afa11f1e507d650bd1fa8c3298c0b6757") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1395244800;
        ...
        assert(block.hashMerkleRoot == uint256("0xed73258af10de50ff686c340fed1d2b75b3f243f8ac952a5855d8efcd9f7438b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // WPCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WPCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WPCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WPCoinMiner(CWallet *pwallet)
{
    printf("WPCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WPCoin-miner");
    ...
    try { loop {
        ...
        printf("Running WPCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WPCoinMiner, pwallet));
}
